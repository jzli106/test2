...
class bst
{
    public:
        ...
        bst(const T &d, size_t c = 1, BST_TRAVERSAL_TYPES t = IN_ORDER);
...
bool bst<T>::remove(const T &d, size_t c)
{
        ...
        cout<<"set temp to root\n";
        //create a temp set the temp's left and right child to root;
    ...
    // find the left or right child
    //WHICH_CHILD child = direction[d < *parent];
    WHICH_CHILD child = direction[d > *parent];

    //place a flag on the to be deleted node
    ...
    *bye -= c;
    ...
    if( bye->getCount() < 1)
    {
           if(bye->childPtr(RIGHT))
           {
                ...
                leftMost = theMost(bye->childPtr(RIGHT), RIGHT);
...
{
    //  check if the node == null
    //      true: return null
    //      false: the result of return the following
                    //true: if the left child of the node == data
                    //      or the right child of the node == data
                                // return the node
                    //fasle: run the find parent function again using left(data < node's data) or right(data > node's data)
                    // if r's left child or right child equal to data return r
                        // if not return an call to find parent with the correct child
                                // determinded by data < r's data
    return !r ? NULL : r->childPtr(LEFT)->getData() == d||r->childPtr(RIGHT)->getData() == d ? r : findParent(r->childPtr(direction[d < *r]),d);                            //
    //return !r ? NULL : r->childPtr(LEFT)->getData() == d||r->childPtr(RIGHT)->getData() == d ? r : findParent(r->childPtr(direction[d < *r]),d);
}
